...
static uint32_t Task_ToneChange_DeltaExecute;
...
extern void Task_Speakerbuzz() {
	...
	if (!Task_SpeakerBuzz_Initialized) {
		...
		Task_SpeakerBuzz_DeltaExecute  = (SysTickFrequency_Nbr / (2 * 833) );
		Task_ToneChange_DeltaExecute = (SysTickFrequency_Nbr / (2 * 833) );
	...
	if ( SysTickCount_Nbr >= Task_ToneChange_DeltaExecute ) {
		if (Task_SpeakerBuzz_DeltaExecute == (SysTickFrequency_Nbr / (2 * 833))) {
			Task_SpeakerBuzz_DeltaExecute = (SysTickFrequency_Nbr / (2 * 444) );
		} else {
			Task_SpeakerBuzz_DeltaExecute = (SysTickFrequency_Nbr / (2 * 833) );
		}

		Task_ToneChange_DeltaExecute = SysTickCount_Nbr + 7500;
	}
